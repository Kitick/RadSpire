[BUG] crafting the sword does not equip it right away
[BUG] shield does nothing
[BUG] cannot open a chest correctly, it just picks up
[BUG] enemies are not persisted

[IMP] Fix the item preview in the crafting UI to show item information
[IMP] Combine crafting lists to one list.
[IMP] Crafting should show what items are present / missing

[IMP] in game explaination for why you are there
[IMP] in game explaination for where resources can be found

[FET] added a radiation bar that takes health away.
[FET] add a radspire to emit radation as you get closer

[FET] add artifacts that boost the player stats
[FET] enemies should drop artifacts when killed

[FET] add equipment menu for armor, and an offhand for sheild
